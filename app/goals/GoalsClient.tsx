...
import { Plus, Target, Check, X } from "lucide-react";
import type { Artifact, Goal, Memory, Task } from "../lib/types";
import { PageShell } from "../components/Shell";
import { formatDate } from "../components/ui";

function progress(goalId: string, tasks: Task[]) {
...
function daysUntil(iso: string | Date | null) {
  if (!iso) return 0;
...
function HeroGoal({
  ...
  const p = progress(goal.id, tasks);
  ...
  const goalArtifacts = artifacts
    ...
    .slice(0, 4);
  const goalNotes = memories.filter((m) => m.type === "agent_note").slice(0, 3);
      ...
      {editing ? (
        <>
          <input
            ...
            className="mt-2 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-2xl font-semibold leading-tight tracking-tight text-zinc-900 outline-none focus:border-zinc-400"
          />
          <textarea
            ...
            rows={3}
            className="mt-2 w-full resize-none rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm leading-relaxed text-zinc-700 outline-none focus:border-zinc-400"
          />
        </>
      ) : (
        <>
          ...
          <div>
            ...
            {editing ? (
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, success_metric: e.target.value })
                }
                className="mt-1 w-full rounded-md border border-zinc-200 bg-white px-2 py-1 text-sm text-zinc-700 outline-none focus:border-zinc-400"
              />
            ) : (
          ...
          {editing ? (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-md bg-zinc-900 px-2.5 py-1 text-xs font-medium text-white hover:bg-zinc-700 disabled:opacity-50"
              >
                <Check size={11} />
                {saving ? "Saving…" : "Save"}
              </button>
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 h-1 overflow-hidden rounded-full bg-zinc-100">
        <div className="h-full bg-zinc-700" style={{ width: `${p.pct}%` }} />
      </div>
